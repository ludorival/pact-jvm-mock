...
internal data class PactToWrite(
    ...
    val pact: Pact
        get() = createPact(
            ...
            provider,
            version,
            descriptions.map { interactionsByDescription.getValue(it).asV4InteractionIfNecessary() }
        ).run {
            if (version == PactSpecVersion.V4) asV4Pact().get() ?: error("There is an issue when converting this pact to V4") else this
        }

    private fun Interaction.asV4InteractionIfNecessary(): Interaction = if (version == PactSpecVersion.V4) asV4Interaction().also {
        it.comments.putAll(comments)
    } else this
    ...
    private fun printDifferences(old: Interaction, current: Interaction): String {
        ...
        return """The interaction with description "${current.description}" has changed generated by test "${current.comments["testname"]}" in file "${current.comments["testfile"]}"
                    |The changes are:
